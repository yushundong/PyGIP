...
class WatermarkGraph:
    ...
    def _generate_wm(self, n, num_features, num_classes):
        ...
        wm_x = torch.tensor(np.random.binomial(1, self.pr, size=(n, num_features)), dtype=torch.float32)
        wm_y = torch.tensor(np.random.randint(low=0, high=num_classes, size=n), dtype=torch.long)
...
class Watermark_sage(Defense):
    def __init__(self, dataset, attack_node_fraction, wm_node=50, pr=0.1, pg=0, device="cpu"):
        ...
        self.model = GraphSAGE(in_channels=self.graph.ndata['feat'].shape[1], hidden_channels=128, out_channels=7)
    ...
    def attack(self):
        # enable_cpu_affinity()
        data_wm = WatermarkGraph(n=self.wm_node, num_features=self.feature_number, num_classes=self.label_number, pr=self.pr, pg=self.pg, device=self.device).graph_wm
